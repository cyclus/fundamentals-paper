...
\begin{itemize}
    ...
    \item \textbf{Recycled Fuel Fabrication} (\Class{cycamore::Fab}): This
        ...
        \TODO{add D-factors citation} in an attempt to match simple neutronics
    ...
    \item \textbf{DU Source} (\Class{cycamore::Source}): This facility provides
...
as waste.  For the modified open case, the \Class{Reactor} offerred spent UOX
into a spent fuel market where it is preferrentially sent to separations, but
spent MOX is still offerred as waste.  In the closed case, the \Class{Reactor}
...
modified open fuel cycle.  Note that the \Class{Reactor} always transmutes
...
does not re-recycle material, it takes the fabrication facility 1 cycle longer
