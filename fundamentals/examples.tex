...
The functionality of the \Cyclus simulator is illustrated by demonstrations
...
advance in a community-driven \textbf{collaborative?} development paradigm. Additionally, simple 
simulations as well as more complex recycling simulations demonstrate that 
\Cyclus possesses fuel cycle simulation capabilities.
...
\end{itemize}
Taken together, these form an ecosystem of capabilites. Over time, this 
...
Because material is tracked as discrete objects, single-pass recycle using \gls{MOX} 
...
\TODO{(no recycle), modified open, and closed (infinite-pass recycle)
variations of}
...
commodity.  The dynamic resource exchange \TODO{acro or not?} enables a reactor to
...
\end{lstlisting}
This results in the material flows in Figure \ref{fig:flowclosed}. Note
that the \Class{BatchReactor} always transmutes fuel into the same
...
(no recycle), modified open, and closed (infinite-pass recycle) \TODO{whats
the diff btw closed and the multi-pass from above? It's referred to as fully
closed multi-pass recycle, which is a bit confusing.} variations of
...
month 300 \TODO{here and below: month or time step?}) in the modified open and closed cases, enough separated fissile
