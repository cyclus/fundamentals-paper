...
\hline
Cycic &  Input control - embedded in Cyclist & \cite{cycic}\\
Cyclist & Interactive data exploration environment & \cite{cyclist} \\
Ciclus & Continuous Integration scripts for \Cyclus & \cite{ciclus}\\
Cycstub & Skeleton for clean slate module development & \cite{cycstub}\\
Cyan & \Cyclus analysis tool & \cite{cyan}\\
Cloudlus & Tools for running \Cyclus in a cloud environment & \cite{cloudlus} \\
\hline
\end{tabularx}
\caption{Many tools outside of the scope of the \Cyclus kernel have been 
developed for improved user, developer, and analyst experinces with \Cyclus.}
\label{tab:coretools}
\end{table}
...
have been \cite{huff_cyder_2013,,streamblender,commodconverter} 
and are being \cite{britelite,utk,mktdriveninst} developed for contribution to the 
