...
both remote and mutiprocess execution on a number of platforms. This section begins by 
describing the general design features that make \Cyclus both flexible and powerful:
cluster-ready software, dynamic libraries, and interchangeable agents. It then introduces
the specific framework of \Cyclus: the region-institution-facility hierarchy, handling
of resources and materials, the Cycamore library of agents, and the experimental toolkit.
...
\acrlong{ABM} is inherently object-oriented because agents represent 
...
any archetype can have access to the \Cyclus agent deployment mechanism.  It is easy
...
decay, etc.) are tracked in detail as they are performed.  This information includes th
agent that newly created resources are created from  
when they are introduced into the simulation.
...
composition is represented by an arbitrarily large list (potentially thousands) of
