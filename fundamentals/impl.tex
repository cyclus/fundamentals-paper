...
Agent-based modeling is inherently object oriented. Since agents are  
...
Agent plugins utilize the generic core API to interact with one another.  For 
...
A key innovation in \Cyclus is implementing this generic API and modular 
...
\begin{figure}[htbp!]
\begin{center}
\includegraphics{agent_uml}
\end{center}
\caption{The inheritance structure in the \Class{Agent} class abstracts away 
unneccesary details while exposing powerful functionality.}
\label{fig:agent_uml}
\end{figure}

In a similar fasion, the \Class{Trader} interface allows agent-agent interaction through the
...
\Cycamore \cite{carlsen_cycamore_2014} contains a number of useful facility, 
region, and institution archetypes. 
...
\end{center}
\caption{A simple once-through fuel cycle possible with only \Cycamore and 
\Cyclus.}
\label{fig:<+label+>}
\end{figure}<++>
