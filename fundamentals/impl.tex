...
processes independent of the simulation logic. To achieve this, agents are
developed to represent facilities, institutions, and regions comprising the
...
studies on \Cyclus' resource exchange optimization solvers. Cyclopts has run over $10^5$ jobs, comprising more than 60,000 total compute hours,
...
\Cyclus achieves this behavior by implementing its generic \gls{API} and
...
The core framework operates
independently from the plug-in libraries, and the APIs are designed to provide archetype developers with the tools needed to collect inputs required by their archetype as well as return outputs. Hence the libraries enable customization and extension of \Cyclus' capabilities independent of the core.
...
\Cyclus implements an \acrlong{ABM} paradigm.  \gls{ABM} provides a natural
...
the developer to implement agents that deploy copies of themselves or
